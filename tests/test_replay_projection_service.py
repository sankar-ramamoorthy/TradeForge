...
from src.services.replay import ReplayProjectionService
...
def _event(event_type: str, offset_minutes: int = 0) -> EventEnvelope:
    return EventEnvelope(
        ...
        timestamp=datetime(2026, 5, 10, 14, 0, tzinfo=UTC)
...
def test_replay_projection_service_satisfies_event_store_port() -> None:
    ...
    service = ReplayProjectionService(event_store)

    projection = service.project()

    assert projection.source_event_count == 2
    assert projection.lifecycle_state is not None
    assert projection.lifecycle_state.current_stage is LifecycleStage.THESIS


def test_replay_projection_service_reads_history_without_appending_events() -> None:
    ...
    assert event_store.read_events() == (_event("decision.trade_idea_created"),)


def test_replay_service_has_no_infrastructure_or_app_dependency() -> None:
