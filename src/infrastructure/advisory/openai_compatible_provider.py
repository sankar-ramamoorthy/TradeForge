...
from src.security.litellm_credential import LITELLM_PROVIDER_ID, LiteLLMCredentialPayload
...
_SYSTEM_PROMPTS: dict[AdvisoryArtifactKind, str] = {
    AdvisoryArtifactKind.REPLAY_SUMMARY: (
        "You are an advisory assistant helping a discretionary trader review a completed "
    ...
    AdvisoryArtifactKind.REVIEW_ASSISTANCE: (
        ...
        "completed trade. Surface patterns in the decision process, identify behavioural "
    ...
    AdvisoryArtifactKind.THESIS_REVIEW: (
        ...
        "and potential blind spots in the reasoning. Surface the questions the operator "
        "may not have considered. Do not validate, endorse, or rank the thesis."
        + _SAFETY_FOOTER
    ),
    AdvisoryArtifactKind.OBSERVATION_GENERATION: (
        "You are an advisory assistant generating structured advisory observations about "
        ...
        "Generate multiple distinct observations — do not collapse into a single summary."
        + _SAFETY_FOOTER
    ),
...
class OpenAICompatibleAdvisoryProvider:
    ...
    def __init__(self, credential: LiteLLMCredentialPayload) -> None:
        self._client = OpenAI(
            base_url=credential.base_url,
            api_key=credential.api_key,
        )
        self._model = credential.default_model
    ...
    def generate(self, request: AdvisoryRequest) -> AdvisoryResponse:
        ...
        model_used = completion.model or self._model
